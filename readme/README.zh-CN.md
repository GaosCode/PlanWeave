...
可以从 [GitHub Releases](https://github.com/GaosCode/PlanWeave/releases) 安装打包版本。受保护的 Desktop Release workflow 生成的产物必须先通过 macOS Developer ID 签名、公证、staple 和安装后启动检查，或 Windows Authenticode 签名、时间戳和安装后启动检查，才能发布。本地 `dist:*` 与 `pack:*` 命令仍明确生成未签名的开发构建。

在 Windows 上，有效的 OV 签名会显示已验证发布者，但不能保证 Microsoft Defender SmartScreen 不再显示信誉提示；新文件或新证书仍可能需要积累信誉。在 macOS 上，只有 packaged build metadata 严格验证为已签名 release channel 时才启用应用内安装。产物验签、凭据轮换和失败排查见 [Desktop 发布手册](../docs/desktop-release-runbook.md)。
