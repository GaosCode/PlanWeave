...
export class CollaborationCaptureRecorder {
  ...
  record(
    stage: CaptureStage,
    options: { peer?: string; pointer?: boolean; durationMs?: number } = {}
  ): void {
    ...
    if (options.peer !== undefined) {
      ...
      if (stage === "renderer_receive") this.pending.set(options.peer, this.now());
    }
  ...
  commit(peer: string): void {
    ...
    this.record("renderer_commit", { peer, durationMs: this.now() - received });
  }
  ...
  snapshot(): CaptureTrace | null {
    ...
    return {
      ...
      samples: this.trace.samples.map((sample) => ({ ...sample }))
    };
  }
}
