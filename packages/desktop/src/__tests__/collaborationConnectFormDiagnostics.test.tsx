...
describe("CollaborationConnectForm connection diagnostics", () => {
  ...
  it("shows identity repair actions when split principals are present", () => {
    ...
    expect(screen.queryByTestId("people-identity-repair-recover")).not.toBeInTheDocument();
  });

  it("shows identity recovery when a historical principal only has an expired token", () => {
    render(
      <CollaborationConnectForm
        api={joinApi()}
        status={{
    ...
    expect(screen.getByTestId("people-identity-repair-recover")).toBeInTheDocument();
  });
