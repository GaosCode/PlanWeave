...
describe("desktop external link handling", () => {
  it("opens the tunnel-client release page in the system browser and denies Electron child windows", async () => {
    ...
    expect(handler({ url: "https://github.com/openai/tunnel-client/releases/latest" })).toEqual({
      action: "deny"
    });
  ...
  it("opens the PlanWeave desktop releases page in the system browser", async () => {
    ...
    expect(handler({ url: "https://github.com/GaosCode/PlanWeave/releases/latest" })).toEqual({
      action: "deny"
    });
    expect(electronMock.shell.openExternal).toHaveBeenCalledWith(
