...
function structuredPayloadText(value: unknown, depth = 0): string {
  ...
  if (Array.isArray(value)) {
    ...
    return value
      .map((item) => `${indent}- ${structuredPayloadText(item, depth + 1).trimStart()}`)
      .join("\n");
  }
...
export function AcpConversationTimeline({
  changeKey,
  timeline,
  t
}: {
  ...
  return (
    <div className="relative min-h-0 flex-1">
      <div
        className="h-full overflow-y-auto rounded-xl border bg-muted/15 p-4"
        ref={viewportRef}
        onScroll={onScroll}
        data-testid="acp-conversation-viewport"
      >
...
export function AcpConversationItems({
    ...
    <div className="space-y-4">
      {visibleTimeline.map((item) =>
        item.kind === "artifact" ? (
          ...
          <details
            className={`${workspace ? "" : "ml-10 "}rounded-lg border bg-muted/40 px-3 py-2 text-xs`}
            key={`output-${item.sequence}`}
          >
            <summary className="cursor-pointer font-medium">
              {item.stream === "terminal" ? t("acpTerminalOutput") : item.stream}
            </summary>
  ...
  function MessageCard({
    ...
    return (
      <article className={`flex gap-3 ${user ? "flex-row-reverse" : ""}`}>
        <div
          className={`mt-1 flex size-7 shrink-0 items-center justify-center rounded-full ${user ? "bg-primary text-primary-foreground" : "border bg-background"}`}
        >
          {user ? <UserIcon className="size-3.5" /> : <BotIcon className="size-3.5" />}
        </div>
        <div className={`max-w-[86%] space-y-1 ${user ? "items-end" : ""}`}>
          <div
            className={`text-[10px] font-medium uppercase tracking-wide text-muted-foreground ${user ? "text-right" : ""}`}
          >
            {user ? t("acpRoleUser") : t("acpRoleAssistant")}
          </div>
          <div
            className={
              user
...
function WorkspaceToolRow({
  ...
  return (
    <div className="group/tool text-xs" data-testid="workspace-tool-event">
      <button
      ...
      >
        ...
        <span className="min-w-0 flex-1 truncate font-medium transition-colors duration-[var(--motion-duration-fast)] group-hover/tool:text-foreground group-focus-within/tool:text-foreground">
          {tool.title}
        </span>
        <span className={tool.status === "failed" ? "shrink-0 text-destructive" : "shrink-0"}>
          {status}
        </span>
        {expandable ? (
          <ChevronRightIcon
            className={`size-3.5 shrink-0 opacity-0 transition-[transform,opacity] duration-[var(--motion-duration-base)] ease-[var(--motion-ease-standard)] group-hover/tool:opacity-100 group-focus-within/tool:opacity-100 motion-reduce:transition-none ${open ? "rotate-90" : ""}`}
          />
        ) : null}
      </button>
      {expandable ? (
...
function ToolPayload({
  ...
  return (
    <div className="space-y-1">
      ...
      <pre
        className={`max-h-64 overflow-auto whitespace-pre-wrap break-words font-mono text-[11px] leading-5 ${presentation === "workspace" ? "" : "rounded-md bg-muted p-2"}`}
      >
        {value}
      </pre>
    </div>
