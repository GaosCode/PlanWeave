...
export function useGraphHistoryActions({
  ...
  const runGraphHistoryCommand = useCallback(
    async (command: "undo" | "redo") => {
      ...
      try {
        ...
        const result =
          command === "undo"
            ? await bridge.undoPlanGraphCommand(canvas)
            : await bridge.redoPlanGraphCommand(canvas);
        if (!result.ok) {
        ...
        if ((overview.activeCanvasId ?? null) !== selectedCanvasId) {
          await openProjectInSession(overview, overview.activeCanvasId, {
  ...
  const handleUndoGraph = useCallback(
    () => runGraphHistoryCommand("undo"),
    [runGraphHistoryCommand]
  );
  const handleRedoGraph = useCallback(
    () => runGraphHistoryCommand("redo"),
    [runGraphHistoryCommand]
  );
