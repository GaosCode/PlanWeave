...
export function readProjectionVersion(db: SqliteDatabase, graphKey: string, projectionName: string, cacheKey: string): PlanGraphProjectionVersion | null {
  const row = db
...
export function writeProjectionVersion(db: SqliteDatabase, graphKey: string, projection: PlanGraphProjectionVersion): void {
  db.prepare(
