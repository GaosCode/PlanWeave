...
import {
  buildProjectExecutionPlanFromContext,
...
async function captureSnapshotPart<T>(
  ...
  try {
    ...
    appendDesktopDiagnostic(
      diagnostics,
      desktopDiagnostic("desktop_snapshot_part_failed", errorMessage(caught), label)
    );
    return null;
  }
}
...
type SnapshotResult<T> = { ok: true; value: T } | { ok: false; error: unknown };
...
function sameWorkspace(left: ProjectWorkspace, right: ProjectWorkspace): boolean {
  return (
    resolve(left.packageDir) === resolve(right.packageDir) &&
...
function selectedGraphContextFromProjectTodoContext(
  ...
  const canvas = context.aggregation.canvases.find((candidate) =>
    sameWorkspace(candidate.workspace, selectedWorkspace)
  );
  if (!canvas) {
  ...
  if (snapshot.error) {
    throw new Error(
      `Selected task canvas '${canvas.canvasId}' execution snapshot failed: ${errorMessage(snapshot.error)}`
    );
  }
...
export async function getDesktopProjectSnapshot(
  ref: DesktopCanvasReference
): Promise<DesktopProjectSnapshot> {
  const diagnostics: ValidationIssue[] = [];
  const selectedWorkspace: SelectedWorkspaceResult = await resolveTaskCanvasWorkspace(
    ref.projectRoot,
    ref.canvasId
  )
  ...
  const projectProjection: SnapshotResult<DesktopProjectProjection> = await captureResult(() =>
    readDesktopProjectProjection(ref.projectRoot)
  );
  ...
  const selectedGraphContext: SnapshotResult<DesktopGraphViewModelContext> = selectedWorkspace.ok
    ? projectTodoContext.ok
      ? await captureResult(async () =>
          selectedGraphContextFromProjectTodoContext(
            projectTodoContext.value,
  ...
  const [
    ...
    captureSnapshotPart(diagnostics, "projectPromptMarkdown", () =>
      readProjectPrompt(ref.projectRoot)
    ),
    captureSnapshotPart(diagnostics, "projectPromptPolicy", () =>
      readProjectPromptPolicy(ref.projectRoot)
    ),
    captureSnapshotPart(diagnostics, "graph", async () =>
      buildGraphViewModel(unwrapSnapshotResult(selectedGraphContext))
    ),
    captureSnapshotPart(diagnostics, "layout", async () => {
      if (selectedGraphContext.ok) {
        return getDesktopLayoutForPackage(
          selectedGraphContext.value.workspace,
          selectedGraphContext.value.manifest
        );
      }
      return getDesktopLayout(getSelectedWorkspace());
    }),
    captureSnapshotPart(diagnostics, "todoGroups", async () =>
      buildTodoGroupsFromContext(unwrapSnapshotResult(projectTodoContext))
    ),
    captureSnapshotPart(diagnostics, "executionPlan", async () =>
      buildProjectExecutionPlanFromContext(unwrapSnapshotResult(projectTodoContext))
    ),
