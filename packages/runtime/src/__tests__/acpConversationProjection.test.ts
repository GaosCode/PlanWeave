...
import { projectAcpConversation, projectAcpTimeline } from "../autoRun/acpConversationProjection.js";
...
describe("ACP conversation projection", () => {
  ...
  it("applies tool updates as replacement state in one renderer-owned timeline item", () => {
    ...
    const events = [
      normalizedRunnerEventSchema.parse({ ...base, sequence: 1, body: {
        kind: "tool_call", callId: "tool-1", title: "Read file", status: "in_progress",
        toolKind: "read",
        content: null,
        rawInput: { content: "README.md", redaction: { classes: [], replaced: 0 } },
        rawOutput: null
      } }),
      normalizedRunnerEventSchema.parse({ ...base, sequence: 2, body: {
        kind: "tool_update", callId: "tool-1", status: "in_progress", title: "Reading README",
        content: { content: "part one", redaction: { classes: [], replaced: 0 } }
      } }),
      normalizedRunnerEventSchema.parse({ ...base, sequence: 3, body: {
        kind: "tool_update", callId: "tool-1", status: "completed",
  ...
  it("distinguishes absent tool content from explicit null replacement", () => {
    ...
    const call = normalizedRunnerEventSchema.parse({ ...base, sequence: 1, body: {
      kind: "tool_call", callId: "tool-1", title: "Run", status: "in_progress",
      content: null,
      rawOutput: { content: "initial", redaction }
    } });
    const absent = normalizedRunnerEventSchema.parse({ ...base, sequence: 2, body: {
      kind: "tool_update", callId: "tool-1", status: "completed"
    } });
    const cleared = normalizedRunnerEventSchema.parse({ ...base, sequence: 3, body: {
      kind: "tool_update", callId: "tool-1", content: null
    } });
  ...
  it("coalesces an out-of-order tool update with its later tool call", () => {
    ...
    const timeline = projectAcpTimeline([
      normalizedRunnerEventSchema.parse({ ...base, sequence: 1, body: {
        kind: "tool_update", callId: "tool-1", title: "Updated title", status: "completed",
        rawOutput: { content: "done", redaction }
      } }),
      normalizedRunnerEventSchema.parse({ ...base, sequence: 2, body: {
        kind: "tool_call", callId: "tool-1", title: "Initial title", toolKind: "execute",
  ...
  it("projects a large chunk stream into one message without quadratic output growth", () => {
    ...
    const events = Array.from({ length: 2_000 }, (_, index) => normalizedRunnerEventSchema.parse({
      ...base,
      sequence: index + 1,
      body: {
        kind: "message", role: "assistant", messageId: "message-1", chunk: true,
        content: "x", redaction: { classes: [], replaced: 0 }
      }
    }));
