...
describe("bounded package content readers", () => {
  it("lists package files with owners and content refs", async () => {
    ...
    expect(result.pagination).toMatchObject({ limit: 2, cursor: null, total: 4, hasMore: true, nextCursor: "next:2" });
  ...
  it("reads package files and prompt sources with bounded content refs", async () => {
    ...
    const manifest = await readPackageFile({ projectRoot: root, path: "manifest.json", maxBytes: 20 });
    const taskPrompt = await readPromptSource({ projectRoot: root, target: "task", taskId: "T-001" });
    const blockPrompt = await readPromptSource({ projectRoot: root, target: "block", blockRef: "T-001#B-001", maxBytes: 10 });
    ...
    expect(blockPrompt.contentRef).toMatchObject({ kind: "prompt_source", path: "nodes/T-001/blocks/B-001.prompt.md" });
  ...
  it("reads rendered prompts and source summaries without exposing local paths", async () => {
    ...
    const rendered = await readRenderedPrompt({ projectRoot: root, ref: "T-001#B-001", maxBytes: 80 });
  ...
  it("rejects package file paths that escape the package root", async () => {
    ...
    await expect(readPackageFile({ projectRoot: root, path: "../project.json" })).rejects.toThrow("must stay inside the package directory");
  });
});
