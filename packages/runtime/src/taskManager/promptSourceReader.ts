...
interface PromptSourceReader {
  readProjectPromptPolicy: () => Promise<ProjectPromptPolicy>;
  readGlobalPrompt: () => Promise<PromptSource>;
  readProjectPrompt: () => Promise<PromptSource>;
  readPackagePrompt: (
    packagePath: string,
...
function createPromptSourceReader(workspace: ProjectWorkspace): PromptSourceReader {
  let projectPromptPolicy: Promise<ProjectPromptPolicy> | undefined;
  const promptFiles = new Map<string, Promise<PromptSource>>();
  const packagePrompts = new Map<string, Promise<PromptSource>>();
...
export { createPromptSourceReader };
export type { PromptSource, PromptSourceReader };
