...
import { registerPackageExportSubcommand } from "./packageExport.js";
...
function formatImportHuman(
  result: Awaited<ReturnType<typeof previewPackageDraftImport>> & { applied?: boolean }
): string {
  return [
...
export function registerPackageImportCommand(program: Command): void {
  const packageCommand = program
    .command("package")
    .description("Import or export PlanWeave packages");
  ...
  addCanvasOption(
    ...
    const result = options.apply
      ? await applyPackageDraftImport(input)
      : await previewPackageDraftImport(input);
  ...
  registerPackageExportSubcommand(packageCommand);
}
