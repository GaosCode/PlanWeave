...
describe("remote agent registry migration v57", () => {
  it("registers the remote agent registry migration", () => {
    expect(migrations).toContainEqual(expect.objectContaining({ version: 57 }));
  });
  ...
  it("creates both tables and the active-grant index on an empty database", async () => {
    ...
    expect(
      database
        .prepare("SELECT 1 FROM pragma_table_info('remote_agents') WHERE name='allow_owner_canvas'")
        .get()
    ).toBeDefined();
  });
