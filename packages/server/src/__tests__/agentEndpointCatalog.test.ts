...
describe("AgentEndpointCatalog", () => {
  ...
  it("enumerates and resolves active Hosts beyond the first repository page", () => {
    ...
    expect(
      state.catalog.resolveForRun(endpointAfterFirstPage.endpointId, "workspace-a", ["acp.codex"], {
        kind: "owner_fleet"
      })
  ...
  it("lists Hosts without workspace mapping as available in fleet and workspace catalogs", () => {
    ...
    expect(workspaceEndpointFor(host)).toMatchObject({ status: "available" });
  });

  it("resolves workspace-canvas runs when a host has no workspace mapping", () => {
    const host = readyHost({
      readinessObservation: {
        workspaceMappings: [],
        acpProfiles: readyHost().readinessObservation!.acpProfiles
  ...
  it.each([
    "missing",
    "invalid"
  ] as const)("does not treat a %s workspace mapping as workspace-canvas unavailability", (status) => {
    const host = readyHost();
  ...
  it("keeps Owner Fleet endpoints available when collaboration Host capacity is occupied", () => {
    ...
    expect(
      state.catalog.resolveForRun(after.endpointId, "workspace-a", ["acp.codex"], {
        kind: "owner_fleet"
      })
    ...
    expect(state.catalog.listProjected({ kind: "owner_workspace" }).items[0]).toMatchObject({
      status: "unavailable",
      unavailableReason: "at_capacity"
    });
  });
  ...
  it("returns one stable unknown error when the endpoint identity disappears", () => {
    ...
    state.setHosts([]);
    expect(() =>
      state.catalog.resolveForRun(endpoint.endpointId, "workspace-a", [], { kind: "workspace" })
    ).toThrowError(new AgentEndpointCatalogError("agent_endpoint_unknown"));
  });
